...
def prepare_data_with_real_status(df):
    ...
    if 'Data Esperada' in df.columns:
        ...
        def ajustar_data_alvo(row):
            data_esperada = row.get('Data Esperada')
            data_prev = row.get('DATA_PREV_SOLUCAO')
            
            # Se não tem Data Esperada, usar Data Prev
            if pd.isna(data_esperada) and pd.notna(data_prev):
                return data_prev
            
            # Se ambas existem e Data Esperada > Data Prev, usar Data Prev
            if pd.notna(data_esperada) and pd.notna(data_prev):
                if data_esperada.date() > data_prev.date():
                    return data_prev
            
            # Caso contrário, manter DATA_ALVO original (Data Esperada)
